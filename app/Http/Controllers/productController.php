    ...
    public function saveproduct(Request $request)
    {
    	...
    	if($image_c)
    	{
    		...
    		if($success)
    		{
    			...
    			$image_o1=$request->file('other_image_1');
		    	if($image_o1)
		    	{
		    		$image_name_1=str_random(20);
		    		$ext_1=strtolower($image_o1->getClientOriginalExtension());
		    		$image_full_name_1=$image_name_1.'.'.$ext_1;
		    		$upload_path_1='image/';
		    		$image_url_1=$upload_path_1.$image_full_name_1;
		    		$success_1=$image_o1->move($upload_path_1, $image_full_name_1);
		    		if($success_1)
		    		{
		    			$data['others_photo_1']=$image_url_1;

		    			$image_o2=$request->file('other_image_2');
				    	if($image_o2)
				    	{
				    		$image_name_2=str_random(20);
				    		$ext_2=strtolower($image_o2->getClientOriginalExtension());
				    		$image_full_name_2=$image_name_2.'.'.$ext_2;
				    		$upload_path_2='image/';
				    		$image_url_2=$upload_path_2.$image_full_name_2;
				    		$success_2=$image_o2->move($upload_path_2, $image_full_name_2);
				    		if($success_2)
				    		{
